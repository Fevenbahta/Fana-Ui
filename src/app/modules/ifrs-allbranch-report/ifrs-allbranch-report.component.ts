...
@Component({
  selector: 'app-ifrs-allbranch-report',
  templateUrl: './ifrs-allbranch-report.component.html',
  styleUrls: ['./ifrs-allbranch-report.component.css']
})
export class IFRSAllbranchReportComponent {
  constructor(
...
transaction:IfrsTransfer
  ...
  statusSearchTerm: string = 'Approved';
...
// this.transactionService.getAllIfrsTransactions().subscribe((t) => {
//   this.reportdata = t.filter(transaction => {

//     this.stopLoadingAnimation(); // Stop loading animation when done
//   // Set loading state to true
    
//     if (transaction.paymentType === 'ATMS'|| transaction.paymentType === 'Derash') {
//         return true; // ATM approved (no branch restriction)
//     } else if (transaction.paymentType === 'RTGS' ) {
//         return true; // RTGS approved and matches the branch
//     }
//     return false; // Exclude other transactions
// });
// const startIndex = (this.currentPage - 1) * this.pageSize;
//         const endIndex = startIndex + this.pageSize;
    
    
//         this.filteredreports =  this.reportdata.filter(ft=>ft.status="Approved");
...
private updateFilteredreports() {
 ...
 buttons = [
  { label: 'Rtgs-Atm AllReport', route: '/RtgsAllReport', role: ['0078','0017','0041', '0048', '0049','Finance'] },
 ];
...
printReceipt(receipt) {
  this.transaction = receipt; // Assuming 'transaction' contains all necessary transaction data.
...
.reciepts{
    ...
    align-items: center; /* Vertically center the logo and title */
                      ...
                      padding:0px;
                            ...
                            .address {
                                text-align: left; /* Align address to the right */
                          ...
                          <div class="transaction-info">
                              <div>
                                  ...
                                  <span><u>${ receipt.tin_No||''}</u></span>
                                          ...
                                          <div>
                                  <span>Payer's Adress/ የግብር ከፋይ አድራሻ:<u>  &nbsp;&nbsp;</u> </span>
    ...
    <span><u> ${this.maskAccountNumber(receipt.account_No)} </u></span>
                            ...
                            <div class="address">
                                   <div><span>Lion International Bank S.C</span></div>
                                             <div><span>TIN No:</span &nbsp;&nbsp;&nbsp;<span><u> 0003229535</u> </span></div>
                                                <div><span> Address:</span>&nbsp;&nbsp;<span><u> Ath. Haile G/Selassie Avenue.Lex Plaza Bldg.</u></span></div>
          ...
          <div class="space">   <div class="amount-in-words">
                         Amount in Words / የገንዘቡ ልክ በፊደል :
                            <span>${this.numberToWords(receipt.amount+receipt.vat +receipt.serviceFee+receipt.vat2 +receipt.serviceFee2)|| 'N/A'} </span>
                        </div>
                    
                        <div class="amount-in-words">
                         Payment Mode / የክፍያ ሁኔታ :
                            <span>${receipt.paymentMethod || 'Acount'}</span>
                        </div>
          </div>
