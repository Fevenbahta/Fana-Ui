...
@Component({
  ...
  styleUrls: ['./branchreport.component.css']
})
...
onMultipleSearch(): void {
  this.filteredreports = this.reportdata.filter(transaction => {
    ...
    const start = this.startDate ? stripTime(new Date(this.startDate)) : null;
    const end = this.endDate ? stripTime(new Date(this.endDate)) : null;

    // Strip time from transactionDate
    const strippedTransactionDate = stripTime(transactionDate);

    // Check if the transaction date falls within the date range (inclusive)
...
ngOnInit(): void {
  ...
  //   this.filteredreports = t.filter(t=>(t.status=='Approved') && t.branch == branchCode);
  //   const lastlist = this.reportdata.filter(t=>(t.status=='Approved')).pop();
  //   this.reportdata.filter(t=>(t.status=='Approved')).unshift(lastlist);
  //   this.filteredreports = this.reportdata.filter(t=>(t.status=='Approved')).slice(startIndex, endIndex);
